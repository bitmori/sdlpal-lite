...
--*/
{
   ...
   const int          iItemsPerLine = 34 / gWordLength;
   const int          iItemTextWidth = 8 * gWordLength + 20;
   const int          iLinesPerPage = 7 - gScreenLayout.ExtraItemDescLines;
   const int          iCursorXOffset = gWordLength * 5 / 2;
   const int          iAmountXOffset = gWordLength * 8 + 1;
   const int          iPageLineOffset = (iLinesPerPage + 1) / 2;
   const int          iPictureYOffset = (gScreenLayout.ExtraItemDescLines > 1) ? (gScreenLayout.ExtraItemDescLines - 1) * 16 : 0;
   ...
   //
   if (!g_fNoDesc && PAL_HasObjectDesc())
   {
      LPCSTR d = PAL_GetObjectDesc(wObject);
      if (d != NULL)
      {
         PAL_DrawObjectDesc(d, gpScreen, 75, 150, 0x3C);
      }
   }
