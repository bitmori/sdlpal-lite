			...
			case ST_LAYOUT:
				...
				else
				{
					...
					if (i >= 1 && i <= (sizeof(SCREENLAYOUT) / sizeof(PAL_POS)))
					{
						...
						{
							gScreenLayoutArray[i - 1] = PAL_XY(x, y);
							if (n == 3) gScreenLayoutFlag[i - 1] = f;
						}
					}
				}
				break;
...
--*/
{
   ...
   if (gpszMsgFile)
   {
	   ...
	   //
	   FILE *fp = UTIL_OpenRequiredFileForMode(gpszMsgFile, "r");
   ...
   else
   {
	   ...
	   //
	   g_TextLib.nWords = (i + (gWordLength - 1)) / gWordLength;
	   if (g_TextLib.nWords < MINIMAL_WORD_COUNT) g_TextLib.nWords = MINIMAL_WORD_COUNT;

	   //
	   // Read the words
	   //
	   temp = (LPBYTE)malloc(gWordLength * g_TextLib.nWords);
	   if (temp == NULL)
	   {
		   fclose(fpWord);
		   fclose(fpMsg);
	   ...
	   memset(temp + i, 0, gWordLength * g_TextLib.nWords - i);
	   ...
	   {
		   int base = i * gWordLength;
		   int pos = base + gWordLength - 1;
		   while (pos >= base && temp[pos] == ' ') temp[pos--] = 0;
		   wlen += PAL_MultiByteToWideChar((LPCSTR)temp + base, gWordLength, NULL, 0) + 1;
	   }
	   ...
	   for (i = 0, wpos = 0; i < g_TextLib.nWords; i++)
	   {
		   ...
		   l = PAL_MultiByteToWideChar((LPCSTR)temp + i * gWordLength, gWordLength, g_TextLib.lpWordBuf[i], wlen - wpos);
