...
--*/
{
   ...
   gAudioDevice.iMusicVolume = gMusicVolume * SDL_MIX_MAXVOLUME / PAL_MAX_VOLUME;
   gAudioDevice.iSoundVolume = gSoundVolume * SDL_MIX_MAXVOLUME / PAL_MAX_VOLUME;
   ...
   //
   gAudioDevice.spec.freq = PAL_SAMPLE_RATE;
   gAudioDevice.spec.format = AUDIO_S16;
   gAudioDevice.spec.channels = PAL_AUDIO_CHANNELS;
   gAudioDevice.spec.samples = PAL_AUDIO_DEFAULT_BUFFER_SIZE;
   ...
   //
   // Initialize the music subsystem (RIX only for DOS version).
   //
   gAudioDevice.pMusPlayer = RIX_Init(UTIL_GetFullPathName(PAL_BUFFER_SIZE_ARGS(0), gpszGamePath, "mus.mkf"));
...
--*/
{
   AUDIO_ChangeVolumeByValue(&gMusicVolume, 3);
   AUDIO_ChangeVolumeByValue(&gSoundVolume, 3);
   gAudioDevice.iMusicVolume = gMusicVolume * SDL_MIX_MAXVOLUME / PAL_MAX_VOLUME;
   gAudioDevice.iSoundVolume = gSoundVolume * SDL_MIX_MAXVOLUME / PAL_MAX_VOLUME;
}
