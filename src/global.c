...
static VOID
PAL_OpenRequiredFiles(
   VOID
)
{
	FILE *fpABC = UTIL_OpenRequiredFile("abc.mkf");
...
--*/
{
   ...
   //
   // Open additional required game data files
   //
   PAL_OpenRequiredFiles();
   ...
   //
   if (!gpszMsgFile) PAL_SetCodePage(PAL_DetectCodePage("word.dat"));
...
)
{
	...
	//
	FILE *fp = UTIL_OpenFileAtPath(gpszSavePath, PAL_va(1, "%d.rpg", iSaveSlot));
	//
...
)
{
	...
	// Try writing to file
	//
	if ((fp = UTIL_OpenFileAtPathForMode(gpszSavePath, PAL_va(1, "%d.rpg", iSaveSlot), "wb")) == NULL)
	{
		return;
	}
