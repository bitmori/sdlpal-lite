...
--*/
{
	...
	while (len > 0)
	{
			...
			if (pRixPlayer->iCurrentMusic == -1 || pRixPlayer->iRemainingFadeSamples <= 0)
			{
				...
				if (pRixPlayer->iNextMusic > 0)
				{
					...
					if (pRixPlayer->iCurrentMusic > 0)
						pRixPlayer->dwStartFadeTime += pRixPlayer->iTotalFadeOutSamples * 1000 / PAL_SAMPLE_RATE;
					else
		...
		//
		int buf_max_len = PAL_SAMPLE_RATE / 70 * PAL_AUDIO_CHANNELS * sizeof(short);
		bool fContinue = true;
			...
			if (pRixPlayer->pos == NULL || pRixPlayer->pos - pRixPlayer->buf >= buf_max_len)
			{
				...
				if (pRixPlayer->resampler[0])
				{
					...
					while (sample_count)
					{
						...
						if (to_write)
						{
							short *tempBuf = (short*)alloca(to_write * PAL_AUDIO_CHANNELS * sizeof(short));
							int temp_buf_read = 0;
							pRixPlayer->opl->update(tempBuf, to_write);
							for (int i = 0; i < to_write * PAL_AUDIO_CHANNELS; i++)
								resampler_write_sample(pRixPlayer->resampler[i % PAL_AUDIO_CHANNELS], tempBuf[temp_buf_read++]);
						}

						int to_get = resampler_get_sample_count(pRixPlayer->resampler[0]);
						if (to_get > sample_count) to_get = sample_count;
						for (int i = 0; i < to_get * PAL_AUDIO_CHANNELS; i++)
							finalBuf[samples_written++] = resampler_get_and_remove_sample(pRixPlayer->resampler[i % PAL_AUDIO_CHANNELS]);
						sample_count -= to_get;
					}
				}
				else
...
--*/
{
	if (object != NULL)
	{
		...
		for (int i = 0; i < PAL_AUDIO_CHANNELS; i++)
...
--*/
{
	...
	if (pRixPlayer->FadeType != RIXPLAYER::FADE_OUT)
	{
		...
		pRixPlayer->iTotalFadeOutSamples = (int)round(flFadeTime / 2.0f * PAL_SAMPLE_RATE) * PAL_AUDIO_CHANNELS;
	...
	{
		pRixPlayer->iTotalFadeInSamples = (int)round(flFadeTime / 2.0f * PAL_SAMPLE_RATE) * PAL_AUDIO_CHANNELS;
	}
...
--*/
{
	...
	if (PAL_USE_SURROUND_OPL)
	{
		switch (PAL_OPL_TYPE)
		{
		case OPL_DOSBOX:
			pRixPlayer->opl = new CSurroundopl(
				new CDemuopl(PAL_OPL_SAMPLE_RATE, true, false),
				new CDemuopl(PAL_OPL_SAMPLE_RATE, true, false),
				true, PAL_OPL_SAMPLE_RATE, PAL_SURROUND_OPL_OFFSET);
			break;
		case OPL_DOSBOX_NEW:
			pRixPlayer->opl = new CSurroundopl(
				new CDBemuopl(PAL_OPL_SAMPLE_RATE, true, false),
				new CDBemuopl(PAL_OPL_SAMPLE_RATE, true, false),
				true, PAL_OPL_SAMPLE_RATE, PAL_SURROUND_OPL_OFFSET);
			break;
		case OPL_MAME:
			pRixPlayer->opl = new CSurroundopl(
				new CEmuopl(PAL_OPL_SAMPLE_RATE, true, false),
				new CEmuopl(PAL_OPL_SAMPLE_RATE, true, false),
				true, PAL_OPL_SAMPLE_RATE, PAL_SURROUND_OPL_OFFSET);
			break;
		}
	...
	else
	{
		...
		{
		case OPL_DOSBOX:
			pRixPlayer->opl = new CDemuopl(PAL_OPL_SAMPLE_RATE, true, PAL_AUDIO_CHANNELS == 2);
			break;
		case OPL_DOSBOX_NEW:
			pRixPlayer->opl = new CDBemuopl(PAL_OPL_SAMPLE_RATE, true, PAL_AUDIO_CHANNELS == 2);
			break;
		case OPL_MAME:
			pRixPlayer->opl = new CEmuopl(PAL_OPL_SAMPLE_RATE, true, PAL_AUDIO_CHANNELS == 2);
			break;
		}
	}
	...
	if (PAL_OPL_SAMPLE_RATE != PAL_SAMPLE_RATE)
	{
		for (int i = 0; i < PAL_AUDIO_CHANNELS; i++)
		{
			pRixPlayer->resampler[i] = resampler_create();
			resampler_set_quality(pRixPlayer->resampler[i], AUDIO_IsIntegerConversion(PAL_OPL_SAMPLE_RATE) ? RESAMPLER_QUALITY_MIN : PAL_RESAMPLE_QUALITY);
			resampler_set_rate(pRixPlayer->resampler[i], (double)PAL_OPL_SAMPLE_RATE / (double)PAL_SAMPLE_RATE);
		}
	}

#if USE_RIX_EXTRA_INIT
	if (gpExtraFMRegs && gpExtraFMVals)
	{
		pRixPlayer->rix->set_extra_init(gpExtraFMRegs, gpExtraFMVals, gExtraFMLength);
	}
#endif
	...
	return (LPAUDIOPLAYER)pRixPlayer;
}
