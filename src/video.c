...
static SDL_Texture *VIDEO_CreateTexture(int width, int height)
{
	...
	ratio *= 1.6f * 10.0f / 16.0f;
	//
	// Check whether to keep the aspect ratio
	//
	if (PAL_KEEP_ASPECT_RATIO && fabs(ratio - 1.6f) > FLT_EPSILON)
	{
		if (ratio > 1.6f)
		{
			texture_height = 200;
...
--*/
{
   ...
   gpWindow = SDL_CreateWindow("Pal", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                               PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT, PAL_VIDEO_INIT_FLAGS | (gFullScreen ? SDL_WINDOW_BORDERLESS : 0) );
   ...
   //
   gpScreenReal = SDL_SetVideoMode(PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT, 8, PAL_VIDEO_INIT_FLAGS);
   ...
   if (gpScreenReal == NULL)
   {
      ...
      gpScreenReal = SDL_SetVideoMode(640, 480, 8,
         SDL_SWSURFACE | (gFullScreen ? SDL_FULLSCREEN : 0));
   }
...
--*/
{
   ...
   //
   if (PAL_DEFAULT_WINDOW_WIDTH == 640 && PAL_DEFAULT_WINDOW_HEIGHT == 400 && (flags & SDL_FULLSCREEN))
   {
      gpScreenReal = SDL_SetVideoMode(640, 480, bpp, flags);
   }
   else if (PAL_DEFAULT_WINDOW_WIDTH == 640 && PAL_DEFAULT_WINDOW_HEIGHT == 480 && !(flags & SDL_FULLSCREEN))
   {
      gpScreenReal = SDL_SetVideoMode(640, 400, bpp, flags);
   }
   else
   {
      gpScreenReal = SDL_SetVideoMode(PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT, bpp, flags);
   }
...
--*/
{
	...
	//
#if SDL_VERSION_ATLEAST(2,0,0)
	SDL_SaveBMP(gpScreen, PAL_CombinePath(0, gpszSavePath, filename));
#else
	SDL_SaveBMP(gpScreenReal, PAL_CombinePath(0, gpszSavePath, filename));
#endif
}
